...
class MainWindow(Screen):
    ...
    def refresh(self):
        ...
        Thread(target=self.refresh_threaded).start()

    def refresh_threaded(self):
        print("started gettin programs")
        Programlar.clear()
        ...
        for i in range(0, len(self.Urller)):
            try:
                ...
                continue
            if (program != None):
                img = Image(source=(os.path.join(os.getcwd(), 'images', program.kanal + "." + 'png')),
                            size_hint=(0.5, 0.5), pos_hint=(0.5, 0.5))
